...
**选择 CLI 而非 HTTP API：** 两者走的是同一个 Gateway codepath（文档原文确认）。CLI 省掉了 SSE 解析、endpoint enable 配置，完成/失败判断靠 exit code 更直接。唯一代价是丢失 streaming 中间进度，但核心需求只要求 received + started + completed 三个节点，CLI 完全满足。
...
|---------------------|--------|---------|---------------|---------|
| `issues` | `assigned` | assignee = `<bot-username>` | `"issue"` | repo, number, sender |
| `pull_request` | `assigned` | assignee = `<bot-username>` | `"pull_request"` | repo, number, sender |
| `issue_comment` | `created` | body 含 `@<bot-username>` | 由 `issue.pull_request` 字段判断 | repo, number, comment_id, sender |
...
**触发 received 标记：** 入队成功后，调用 GitHub Writer 写一个轻量 reaction，而不是新增一条 `Task received` comment。`mention` 事件 react 在触发 comment 上；`assignment` 事件 react 在 issue/PR body 上。

**当前语义约束：** `eyes` 表示“bridge 已看到并接单”，不是“仍在处理中”。任务开始、完成、失败后都**不会自动移除**这个 reaction；运行态仍以 `started` / `done` / `failed` comments 为准。
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  repo_full       TEXT NOT NULL,           -- "example-org/example-repo"
  ...
  triggered_by    TEXT NOT NULL,           -- GitHub username（用于状态回写）
...
function nextTask(): Task | null {
  ...
  if (laterCount > 0) {
    ...
    // 不写 GitHub comment，避免刷屏。入队时只打 received reaction。
    return nextTask();
  }
...
**跳过规则细节：**
- 跳过时不写 GitHub comment（入队时只打 received reaction），只记 event_log
- 不同 resource 之间不受影响
...
- **received**：webhook 入队后立即写 reaction（跟调用方式无关）
...
**Prompt 构建：** Bridge 只传路由指令，不传 issue 内容。`--message` 的值是一句简短的路由信息（如"你被 assign 到 example-org/example-repo#42，用 gh CLI 获取详情并开始工作"），agent 自己负责 fetch 最新的 issue 上下文。
...
reaction: eyes
target:
- mention -> triggering issue comment
- assignment -> issue / PR body
```
...
<dict>
  <key>Label</key>
  <string>com.example.openclaw-bridge</string>
  <key>ProgramArguments</key>
  <array>
    ...
    <string>/Users/agent/openclaw-bridge/config.json</string>
  </array>
...
**级别：** Org level（目标 GitHub 组织）

**URL：** `https://bridge.your-domain.com/hooks`

**Content type：** `application/json`

**Secret：** 生成一个随机 secret，同时配在 GitHub 和 Bridge 的运行配置中（环境变量或 `config.json`）
...
**由 Bridge 做二次过滤：** org 下所有 repo 的事件都会发过来，Bridge 在代码层面判断 assignee 或 @mention 是否是目标 bot username，不是则忽略。
...
1. 环境变量启动：
   `GITHUB_TOKEN=... GITHUB_WEBHOOK_SECRET=... bun run index.ts`
2. JSON 配置文件启动：
   `bun run index.ts --config ./config.json`
   `bun run index.ts ./config.json`

优先级为：**JSON 配置文件 > 环境变量 > 代码默认值**。若提供 JSON 配置文件，未知字段会直接报错并拒绝启动。
...
```typescript
{
  ...
  "githubWebhookSecret": "__REPLACE__",
  "botUsername": "github-bot",
  "openclawBin": "openclaw",
  "openclawAgentId": "swe",
  "openclawHome": "/Users/agent",
  "taskTimeoutMs": 1800000,
  "gracefulKillMs": 5000,
  "pollIntervalMs": 5000,
  "maxStdoutBytes": 102400
}
```
...
- secret 只应存在于 GitHub org webhook 配置，以及 Bridge 运行时配置载体中（launchctl 环境变量或 `config.json`）
- 若使用 `config.json`，文件权限设为 `600`，只有运行用户可读
...
1. 远端宿主机上 `openclaw` CLI 可用且 Gateway daemon 运行中
2. `openclaw agent --agent swe --message "ping"` 能正常执行并返回
3. Bot GitHub account 的 PAT（需要 `repo` scope）
4. 在目标 GitHub 组织创建 webhook 的管理员权限
