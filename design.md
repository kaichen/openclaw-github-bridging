...
GitHub (org-level webhook)
    ...
    ▼
Cloudflare Tunnel (bridge.your-domain.com/webhook/github)
    │
    │  HTTP → localhost:3847
    ▼
┌─────────────────────────────────────────────┐
...
**触发 ack：** 入队成功后，调用 GitHub Writer 在 issue 上写 comment。ack 中 quote 原始 comment 的前几行，让团队知道 agent 在响应哪条指令。
...
async function callOpenClaw(task: Task): Promise<{ exitCode: number; stdout: string; prUrl: string | null }> {
  ...
  if (exitCode !== 0) {
    throw new Error(`openclaw agent exited with code ${exitCode}: ${stderr || stdout}`);
  }

  const prUrl = extractPrUrl(stdout);
  return { exitCode, stdout, prUrl };
}
```
...
页面 JS 每 10 秒 fetch 刷新。新增：每条任务可展开查看完整 stdout（存在 SQLite 中）。
...
<dict>
  ...
  <array>
    ...
    <string>run</string>
    <string>/Users/agent/openclaw-bridge/src/index.ts</string>
  </array>
...
**URL：** `https://bridge.your-domain.com/webhook/github`
...
export const config = {
  ...
  webhookPath: "/wh-<random>/github",  // 隐蔽路径，与 GitHub webhook URL 一致
...
openclaw-bridge/
├── src/
│   ├── index.ts              # Bun HTTP server 入口 + scheduler 启动
│   ├── env.ts                # 配置项
│   ├── db.ts                 # SQLite 初始化 + 查询函数
│   ├── webhook.ts            # HMAC 验证 + 事件解析 + 过滤
│   ├── dedup.ts              # 去重 + 入队逻辑
│   ├── scheduler.ts          # 串行调度循环
│   ├── openclaw-cli.ts       # Bun.spawn openclaw agent + 超时 + stdout 采集
│   ├── github-writer.ts      # GitHub comment 回写
│   └── dashboard.ts          # /api/tasks JSON + 静态 HTML 路由
├── static/
...
| OpenClaw CLI 不可执行（ENOENT） | 标记 `failed`，error_message 记录"openclaw binary not found"，回写 GitHub |
| OpenClaw CLI 非零退出 | 标记 `failed`，记录 exit code + stderr，回写 GitHub |
| OpenClaw 执行超时（30 min） | SIGTERM → 等 5 秒 → SIGKILL，标记 `failed`，回写 GitHub |
...
- **隐蔽路径：** webhook URL 不使用 `/webhook` 等常见路径，改用含随机前缀的路径（如 `/wh-<random>/github`），降低被扫描发现的概率。配置值在 `config.webhookPath` 中管理。
